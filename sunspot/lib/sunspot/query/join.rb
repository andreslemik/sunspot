  ...
  module Query
    ...
    class Join < AbstractFulltext
      ...
      def to_params
        params = { q: @keywords }
        params[:fl] = '* score'
        params[:qf] = @fulltext_fields.values.map(&:to_boosted_field).join(' ')
        params[:defType] = 'join'
      ...
      def to_subquery
        ...
        q_name = "q#{@target.name}#{object_id}"

        {
          :q => "_query_:\"{!join from=#{@from} to=#{@to} v=$#{q_name}}\"",
          q_name => "_query_:\"{!field f=type}#{@target.name}\" AND _query_:\"{!edismax #{options}}#{keywords}\""
        }
      end
      ...
      #
      def add_boost_query(factor); end

      #
      # Add a boost function
      #
      def add_boost_function(function_query); end
      ...
      #
      def add_phrase_field(field, boost = nil); end
      ...
      #
      def add_highlight(fields = [], options = {}); end
    end
  end
end
