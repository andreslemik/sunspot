...
describe 'search faceting' do
  ...
  context 'date or time json facet' do
    ...
    it 'facets properly with the range specified as time_range' do
      time_range = [Time.utc(2020, 4, 1), Time.utc(2020, 7, 31)]
      search = Sunspot.search(Post) do
        with :blog_id, 1
        json_facet :published_at, time_range: time_range, gap: 1, gap_unit: 'MONTHS'
      end
      expected_rows = [
        { count: 1, value: Time.utc(2020, 4, 1).iso8601 },
        { count: 1, value: Time.utc(2020, 5, 1).iso8601 },
        { count: 2, value: Time.utc(2020, 6, 1).iso8601 },
        { count: 1, value: Time.utc(2020, 7, 1).iso8601 }
      ]
