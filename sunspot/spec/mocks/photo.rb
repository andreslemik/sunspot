...
Sunspot.setup(PhotoContainer) do
  ...
  join(:photo_rating, target: 'Photo',   type: :trie_float,
                      join: { from: :photo_container_id, to: :id }, as: 'average_rating_ft')
  join(:caption,      target: 'Photo',   type: :text, join: { from: :photo_container_id, to: :id })
  join(:description,  target: 'Photo',   type: :text,
                      join: { from: :photo_container_id, to: :id }, prefix: 'photo')
  join(:published,    target: 'Photo',   type: :boolean,
                      join: { from: :photo_container_id, to: :id }, prefix: 'photo')
  join(:description,  target: 'Picture', type: :text,
                      join: { from: :photo_container_id, to: :id }, prefix: 'picture')
  join(:published,    target: 'Picture', type: :boolean,
                      join: { from: :photo_container_id, to: :id }, prefix: 'picture')
end
